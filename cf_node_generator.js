...
const CUSTOM_SOURCE = String(config.custom_source || config.CUSTOM_SOURCE || '').trim();
...
if (SOURCE_TYPE === 'random') {
    const ispName = { cf: '官方优选', ct: '电信优选', cu: '联通优选', cmcc: '移动优选', other: '其他优选', custom: '自定义优选' }[ISP];
...
// ================= 节点缓存与合成函数 =================
function saveGeneratedNodes(ipList) {
    if (ipList.length > 0) {
        // 保留前 10 个最优质的 IP
        ...
        bestIPs.forEach((ip, index) => {
            const ispMark = CUSTOM_SOURCE ? "自定义" : (ISP_NAME_MAP[ISP] || ISP.toUpperCase());
            let remarkStr = '';
            if (CUSTOM_SOURCE) {
                remarkStr = `CF-自定义优选-${index + 1}`;
...
if (ISP === 'custom') {
    ...
    const handleRawText = (text) => {
        ...
        console.log(`🎯 [自定义优选] 成功提取到 ${ipList.length} 个合法 IP。`);
        saveGeneratedNodes(ipList);
    };
    ...
    if (source.startsWith('http://') || source.startsWith('https://')) {
        ...
        $httpClient.get({
            ...
            if (err || !response || response.status !== 200 || !data) {
                console.log("❌ [自定义优选] 远程拉取失败！");
                $notification.post("CF 优选生成器", "自定义获取失败", "远程自定义 IP 列表拉取失败，请检查 URL。");
                $done();
            } else {
                handleRawText(data);
            }
    ...
    $httpClient.get({
        ...
        if (response.status === 200 && data) {
            ...
            $notification.post("CF 优选生成器", "生成 0 个节点", `服务器响应异常，状态码: ${response ? response.status : 'unknown'}`);
            $done();
        }
    });
}
