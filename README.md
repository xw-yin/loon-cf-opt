...
- 生成的节点会以国家代码开头，例如 `HK-其他-列表-1`。数据源未提供国家代码时，脚本会通过 Loon 内置 GeoIP 补齐，查询失败则标记为 `UNK`。
...
#### 🌟 方式 A：配合插件配置面板（推荐）
1. 在插件面板填表并保存后。
2. 打开 **Loon** -> 找到 **“配置”** -> 点击 **“订阅（Subscriptions）”** -> 点击右上角 `+`。
3. 链接输入框中填入：
   ```url
   http://httpbin.org/cf_sub
   ```
4. 点击保存并手动更新，Loon 会自动读取插件面板里的配置填入节点。

#### ⚡ 方式 B：极速 URL 传参（100% 避坑，支持多订阅）
如果你的 Loon 插件系统遇到了占位符未替换的 Bug（例如节点中显示 `{UUID}` 或 `NaN`），或者你想**同时配置多套 Worker/协议 订阅**，你可以**完全不用插件面板**，直接将参数拼在订阅链接后面：
1. 直接在 Loon 中添加节点订阅，链接填入：
   ```url
   http://httpbin.org/cf_sub?uuid=你的UUID&host=你的worker域名&path=/video&port=443&protocol=vless&source_type=random&isp=ct
   ```
   *(参数说明：支持 `uuid`/`host`/`path`/`port`/`protocol`/`source_type`/`isp` 等，键名不区分大小写)*
2. 保存并手动点击更新订阅。
